#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into an agent's skills directory.

Defaults to Codex ($CODEX_HOME/skills). Use --agent claude or --agent bob to
target Claude Code (~/.claude/skills) or IBM Bob (~/.bob/skills) instead.
"""
...
def _bob_home() -> str:
    return os.environ.get("BOB_HOME", os.path.expanduser("~/.bob"))
...
def _default_dest(agent: str) -> str:
    homes = {"codex": _codex_home, "claude": _claude_home, "bob": _bob_home}
    return os.path.join(homes[agent](), "skills")
...
def _parse_args(argv: list[str]) -> Args:
    ...
    parser.add_argument("--dest", help="Destination skills directory (overrides --agent)")
    parser.add_argument(
        "--agent",
        choices=["codex", "claude", "bob"],
        default="codex",
        help="Target agent when --dest is not given: codex ($CODEX_HOME/skills), "
        "claude (~/.claude/skills), or bob (~/.bob/skills)",
    )
    parser.add_argument(
