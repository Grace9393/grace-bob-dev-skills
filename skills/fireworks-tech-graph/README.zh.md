...
同一份 SKILL.md 同时支持 **Claude Code** 和 **IBM Bob**，安装时选择目标 agent 即可：

```bash
# 交互式（按提示选择 agent）
npx skills add yizhiyanhua-ai/fireworks-tech-graph

# Claude Code（项目级 .claude/skills/，加 -g 装到 ~/.claude/skills/）
npx skills add yizhiyanhua-ai/fireworks-tech-graph -a claude-code

# IBM Bob（项目级 .bob/skills/，加 -g 装到 ~/.bob/skills/）
npx skills add yizhiyanhua-ai/fireworks-tech-graph -a bob
```

**手动安装（无 npx）：** 把本仓库文件夹复制到 `~/.claude/skills/fireworks-tech-graph/`（Claude Code）或 `~/.bob/skills/fireworks-tech-graph/`（IBM Bob）。下方的 Python 渲染依赖对两种 agent 都适用。
